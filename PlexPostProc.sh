...
# The if selection statement proceeds to the script if $1 is not empty.
   ...
   $TRANSCODEVIDEO --abr --target big --quick --prefer-ac3 "$FILENAME" -o "$TEMPFILENAME" | tee /var/log/plexpostproc-out.log || fatal "transcode-video has failed. Is it installed?"
